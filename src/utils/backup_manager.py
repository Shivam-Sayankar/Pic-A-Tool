...
def restore_exif_data(preview_window, progress_bar, backup_file_path, parent_tab):
    
    try:

        backup_data = load_pickle(backup_file_path)

        # Proceed only for a valid backup file
        if ("backup_metadata" in backup_data) and ("files" in backup_data):
            ...
            backup_date = base["backup_date_time"]
            original_path = base['original_path']
            backup_type = base["backup_type"]
            num_of_files = base["total_files_processed"]

            backup_info = (
                f"> Backup Date: {backup_date}\n"
                f"> Original Path: {original_path}\n"
                f"> Backup Type: {backup_type}\n"
                f"> Count of Backed up files: {num_of_files}\n"
            )
            backed_up_files = "\n\t".join(backup_data["files"].keys())

            confirmation = confirmation_popup(
                parent_tab, 
                "Proceed with the selected backup file?",
                f"{backup_info}\nFiles:\n\t{backed_up_files}"
            )
            ...
            if confirmation:
                ...
                for i, file in enumerate(all_files):
                    ...
                    preview_window.insert("end", f"Restoring Exif data for ({i+1}/{total_files}): {file}... ")
                ...
                preview_window.insert("end", "~~ Restoration Cancelled ~~")
...
def threaded_restore_exif_data(preview_window, progress_bar, backup_file_path, parent_tab):
